...
exports.createSchoolAdmin = async (req, res) => {
    ...
    try {
        ...
        const admin = new User({ email, school: schoolId, username, role: 'schooladmin', isVerified: true, isApproved: true });
        const resetToken = admin.getResetPasswordToken();
        await admin.save({ validateBeforeSave: false });

        const resetUrl = `${process.env.FRONTEND_URL_EMAIL}/reset-password/${resetToken}`;
        await sendEmail({
            to: admin.email,
            subject: 'You have been invited as a School Administrator',
            html: `<p>Hello ${admin.username},</p><p>You have been invited to become a school administrator for ${school.name} on the XPARK platform.</p><p>Please click the link below to set your password and access your account:</p><a href="${resetUrl}">Set Your Password</a><p>This link will expire in 10 minutes.</p>`
        });
        
        backendLogger.success('School admin created and invitation sent.', { context, correlation_id, details: { adminId: admin._id, schoolId } });
