                    ...
                    <div className="relative p-8 mb-12 rounded-2xl overflow-hidden h-96 flex flex-col justify-between" style={{ backgroundImage: `url(${heroGame.imageUrl || bg})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                        ...
                        <div className="relative z-10 flex justify-between items-start">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="relative"><select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="bg-transparent border border-border rounded-lg py-2 pl-4 pr-10 text-sm text-white appearance-none focus:outline-none focus:border-gray-400 w-40"><option value="All" className="bg-card">All Categories</option>{categories.slice(1).map(cat => <option key={cat} value={cat} className="bg-card">{cat}</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" size={18} /></div>
                            </div>
                        </div>
                        <div className="relative z-10 max-w-2xl">
                            ...
                            <div className="flex gap-4">
                                ...
                                >
                                    <button className="flex items-center bg-black/30 border border-white/20 backdrop-blur-sm text-white font-medium py-3 px-8 rounded-2xl hover:bg-black/50 hover:border-white/30 transition-all duration-300 disabled:opacity-50" disabled={!heroGame.gameUrl || heroGame.isComingSoon}><Play size={20} className="mr-3 fill-white" /> Play</button>
                                </Link>
