...
import { Search, ArrowUp, ArrowDown, Star } from 'lucide-react'; // <-- Import Star icon
...
const GamesProgressPage = () => {
    ...
    // --- THIS IS THE FIX (Part 1): Default sort by new averageRating field ---
    ...
    return (
        <div className="p-8 text-foreground">
            <div className="mb-6 flex justify-between items-center">
                <h1 className="text-2xl font-bold">Game Progress</h1>
                    ...
                    <thead className="border-b border-border">
                        <tr>
                            ...
                            <th onClick={() => handleSort('category')} className="p-4 text-left text-sm font-medium text-muted-foreground cursor-pointer select-none">
                             ...
                             {/* --- THIS IS THE FIX (Part 2): Add Rating column header --- */}
                            <th onClick={() => handleSort('averageRating')} className="p-4 text-left text-sm font-medium text-muted-foreground cursor-pointer select-none">
                                <div className="flex items-center gap-2 hover:text-foreground">Rating{sortKey === 'averageRating' && (sortDirection === 'asc' ? <ArrowUp size={14} /> : <ArrowDown size={14} />)}</div>
                            </th>
                    ...
                    <tbody>
                        {filteredAndSortedGames.map((game) => (
                            <tr key={game._id} className="border-b text-lg border-border last:border-b-0 hover:bg-accent/50">
                                ...
                                {/* --- THIS IS THE FIX (Part 3): Display Rating data --- */}
                                <td className="p-4 text-muted-foreground">
                                    <div className="flex items-center gap-1.5">
                                        <Star size={16} className="text-yellow-500 fill-yellow-500" />
                                        <span>{parseFloat(game.averageRating) > 0 ? game.averageRating : 'N/A'}</span>
                                    </div>
                                </td>
