...
const MyGames = () => {
  ...
  useEffect(() => {
    const context = 'MyGames.useEffect';
    logger.startNewTrace();
    const fetchGames = async () => {
      try {
        const gamesData = await api('/games');
        setAllGames(gamesData);
        logger.info('Fetched all games for MyGames page.', { context });
      } catch (err) {
        setError(err.message || 'Failed to fetch games.');
        logger.error('Failed to fetch all games for MyGames page.', { context, details: { error: err.message } });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleGameUpdate = (updatedGame) => {
    setAllGames((prevGames) => prevGames.map((g) => (g._id === updatedGame._id ? updatedGame : g)));
    if (selectedGame && selectedGame._id === updatedGame._id) setSelectedGame(updatedGame);
  };
            ...
            <div className="relative z-10 flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <input type="text" placeholder="Search" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-48 bg-secondary/80 border border-border rounded-lg py-2 pl-4 pr-10 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-violet-500 placeholder:text-muted-foreground" />
