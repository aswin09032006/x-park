...
const UserSidebar = () => {
    ...
    if (!user) {
        return (
            <div className="w-80 flex-shrink-0 bg-card p-6 rounded-lg">
                <p className="text-center text-muted-foreground">Loading user...</p>
            </div>
        );
    }
    
    // NOTE: Counts are static as this functionality is not yet built on the backend.
    const infoLinks = [
    ...
    return (
        <aside className="w-80 flex-shrink-0 bg-card text-card-foreground p-6 rounded-lg self-start">
            <div className="flex flex-col items-center text-center">
                ...
                <p className="text-muted-foreground text-sm">{user.school || 'School: Not Set'}</p>
            </div>
