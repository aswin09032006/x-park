...
const GameModal = ({ game, isOpen, onClose, onGameUpdate  }) => {
    ...
    const handleRatingSuccess = (updatedGame) => {
        onGameUpdate(updatedGame); // Notify parent page of the update
    ...
    return (
        ...
        >
            <div 
                className="relative bg-card text-card-foreground rounded-2xl w-full max-w-lg overflow-hidden border border-border shadow-2xl animate-in fade-in zoom-in-95"
            >
                <button 
                    onClick={onClose} 
                ...
                <div className="p-8 pt-0 -mt-10 relative z-0">
                    ...
                    <p className="text-muted-foreground mb-6 leading-relaxed line-clamp-4">{description}</p>
                    ...
                    <div className="flex items-center justify-between gap-4 mb-8">
                     ...
                     {/* --- ADJUST GRID AND HIDE RATING STAT IF COMING SOON --- */}
                     <div className={`grid ${isComingSoon ? 'grid-cols-2' : 'grid-cols-3'} gap-6 text-sm`}>
                        <div>
                        ...
                        <div>
                            <p className="text-muted-foreground text-xs uppercase tracking-wider mb-2">Sponsor</p>
                            <div className="flex items-center gap-2">
                                <Users className="text-cyan-400" size={18} />
                                <span className="text-card-foreground font-semibold">{sponsor}</span>
                            </div>
                        </div>
                    </div>
                </div>
